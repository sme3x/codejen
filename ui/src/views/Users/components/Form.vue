...
async function getDetails() {
	loading.value = true
	let res = await UsersModel.api.show(props.id!)
	loading.value = false
	return res.data
}
...
async function create() {
	...
	try {
		...
		let res = await UsersModel.api.store(params)
...
async function update() {
	...
	try {
		let params = structuredClone(toRaw(form.value))
		Object.assign(params, props.forceValues ?? {})
		let res = await UsersModel.api.update(props.id!, params)
		await attachToAll(res.data.id)
		emit('updated')
		return res.data
	} catch (error: any) {
...
async function attachToAll(id: string | number) {
	...
	for (let [key, value] of Object.entries(props.attachTo)) {
		loading.value = true
		try {
			await UsersModel.api.updateRelation(id, key, {
				method: value.method,
				params: [value.id],
			})
		} finally {
